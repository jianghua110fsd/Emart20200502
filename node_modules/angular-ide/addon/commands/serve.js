...
module.exports.patchServeCommand = function patchServeCommand(cliVersion) {
  ...
  if (semver.satisfies(semver.coerce(cliVersion), '>=8.0.0') && !process.env['SKIP_DISABLE_IVY']) {
    try {
      const perform = require('@angular/compiler-cli/src/perform_compile');
      const originalReadConfiguration = perform.readConfiguration;
      perform.readConfiguration = function readConfiguration () {
        const result = originalReadConfiguration.apply(this, arguments);

        result.options.enableIvy = false;
        return result;
      };
    } catch (e) {
      // do nothing
